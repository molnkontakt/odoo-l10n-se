...
|---|---|---|---|
| Bankgiro | `ClrSysMmbId` SESBA / 9900 | `Othr/Id` + `SchmeNm/Prtry` **BGNR** | OCR as `Strd/CdtrRefInf` SCOR, otherwise `Ustrd` |
...
date), charge bearer `SHAR`, no postal addresses. Initiating party and debtor are identified by
the customer id with `SchmeNm/Cd` **BANK**. Only SEK.

Verified against the bank's samples; run your own files through the bank's test service before
going live.
